...
public class UserController {
	...
	public @ResponseBody HashMap<String, Object> UserLogin(HttpServletRequest request, UserDTO userdto) {
		...
		if (result != null) {
			// password 일치 확인
			String pwd = ((UserVO)result).getPwd();
	...
	public @ResponseBody int registerIdCheck(HttpServletRequest request, @RequestParam HashMap<String, String> obj) {
		int response = -1;
		String id = obj.get("id");
		String result = (String)loginService.IdCheckService(id);
		logger.info("ID Check Controller. ID is " + id + ", QUERY RESULT : " + result);
		if(result == null) {
			result = "";
		}
		if("".equals(result)) { response = 0; }
		else { response = 1; }
		return response;
	}
}
